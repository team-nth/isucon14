...
func appGetNotification(w http.ResponseWriter, r *http.Request) {
	...
	for loop > 0 {
		...
		func() {
			...
			if rows, err := db.Queryx("SELECT * FROM ride_statuses WHERE ride_id = ? AND app_sent_at IS NULL ORDER BY created_at ASC", ride.ID); err == nil {
			...
			fare, err := calculateDiscountedFare(ctx, tx, user.ID, ride, ride.PickupLatitude, ride.PickupLongitude, ride.DestinationLatitude, ride.DestinationLongitude)
			if err != nil {
				slog.Error("error calculateDiscountedFare", err)
				time.Sleep(errorSleep)
				return
			}

			baseResponse := &appGetNotificationResponseData{
				RideID: ride.ID,
			...
			if ride.ChairID.Valid {
				chair := &Chair{}
				if err := tx.GetContext(ctx, chair, `SELECT * FROM chairs WHERE id = ?`, ride.ChairID); err != nil {
					slog.Error("error SELECT chairs", err)
					time.Sleep(errorSleep)
					return
				}
				...
				if err != nil {
					slog.Error("error getChairStats", err)
					time.Sleep(errorSleep)
					return
				}

				baseResponse.Chair = &appGetNotificationResponseChair{
					ID:    chair.ID,
					Name:  chair.Name,
					Model: chair.Model,
					Stats: stats,
			...
			for _, yetSentRideStatus := range yetSentRideStatuses {
				baseResponse.Status = yetSentRideStatus.Status
...
func appGetNearbyChairs(w http.ResponseWriter, r *http.Request) {
	...
	err = tx.SelectContext(
		...
		c.is_active
		AND c.is_completed
		AND ST_Within(c.point, ST_GeomFromText('%s'))
	...
	for _, result := range results {
		nearbyChairs = append(nearbyChairs, appGetNearbyChairsResponseChair{
			...
			CurrentCoordinate: Coordinate{
				Latitude:  int(result.Latitude.Int32),
				Longitude: int(result.Longitude.Int32),
			},
		})
	}
	...
	// 	if calculateDistance(coordinate.Latitude, coordinate.Longitude, int(chair.Latitude.Int32), int(chair.Longitude.Int32)) <= distance {
	// 		nearbyChairs = append(nearbyChairs, appGetNearbyChairsResponseChair{
	// 			ID:    chair.ID,
	// 			Name:  chair.Name,
	// 			Model: chair.Model,
	// 			CurrentCoordinate: Coordinate{
	// 				Latitude:  int(chair.Latitude.Int32),
	// 				Longitude: int(chair.Longitude.Int32),
	// 			},
	// 		})
	// 	}
	// }
